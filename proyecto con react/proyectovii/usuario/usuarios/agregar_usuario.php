...
require_once '../includes/php/limpiar.php'; // Funciones de limpieza
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($stmt_check->rowCount() > 0) {
            ...
            $stmt_usuario = $db_con->prepare("INSERT INTO usuarios (nombre_usuario, email, user_pass, id_rol) VALUES (:nombre_usuario, :email, :user_pass, :id_rol)");
            $stmt_usuario->bindParam(':nombre_usuario', $nombre_usuario);
            $stmt_usuario->bindParam(':email', $email);
            $stmt_usuario->bindParam(':user_pass', $password);
